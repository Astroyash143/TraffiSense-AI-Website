  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        {[
          { i: GraduationCap, t: "Learning License", d: "Apply for fresh learner's license", a: "Start application" },
          { i: FileText, t: "Driving License", d: "Convert LL to permanent DL", a: "Begin process" },
          { i: RefreshCw, t: "Renewal", d: "Renew before expiry to avoid penalty", a: "Renew now" },
        ].map(s => (
          <div key={s.t} className="glass-strong rounded-2xl p-6 hover:neon-border-cyan transition-all">
            ...
            <button className="mt-4 w-full px-4 py-2 rounded-xl bg-gradient-to-r from-primary to-secondary text-primary-foreground text-sm font-medium">{s.a}</button>
          </div>
        ))}
      </div>
      <div className="mt-6 grid lg:grid-cols-2 gap-4">
        ...
        <div className="glass-strong rounded-2xl p-6">
          ...
          <a href="https://parivahan.gov.in/" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1 text-sm text-secondary hover:underline">Open Parivahan Portal <ExternalLink className="h-3 w-3"/></a>
        </div>
      </div>
    </div>
  );
